...
ImageSequence.prototype.init = function () {
  if(this.mode === 'sequence') {
    this.preloader();
    if(!_this.shouldPlay) {
      _this.drawCanvas(_this.images[0]);
    }
  } else {
...
ImageSequence.prototype.drawCanvas = function (src) {
  ...
  _this.canvas.width = targetWidth;
  _this.canvas.height = windowHeight;
  ...
  _this.context.drawImage(
    ...
    (_this.canvas.width - frameWidth) / 2, // X position - center it in the canvas
    (_this.canvas.height - frameHeight) / 2, // Y position - 0px from the top
...
ImageSequence.prototype.drawCurrentFrame = function () {
  ...
  if(this.mode === 'sequence') {
    ...
    _this.drawCanvas(_this.images[counter]);
